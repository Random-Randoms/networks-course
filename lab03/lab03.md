...
![alt text]({B548ED46-4154-4C73-8CC4-C2550B2A079F}.png)

Неудачный запрос

![alt text]({04A08C0F-0BD5-4124-B24C-E71A3B3EC482}.png)
...
![alt text]({C2D05084-BAD6-427D-AAFD-32B92AEA34D3}.png)

Неудачный запрос

![alt text]({09506564-947C-403C-ABD1-CB50680F08C5}.png)
...
#### Решение
Сначала до $\frac{56B}{128KBit/s} \approx 3.4 ms$ происходит сборка пакета.
Потом $5ms$ идет задержка распространения.
Пакет передается $\frac{56B}{1MBit/s} \approx 0.4ms$.
Далее он декодируется еще $3.4ms$. Итого $12.2ms$.
...
#### Решение
В буфере в среднем $10$ пакетов, поэтому $N=11$.
Задержка передачи пакетов составляет $10ms$ (так как канал может передавать 100
пакетов в секунду).
Тогда суммарная задержка $d = 10ms + 10ms = 20ms$.
А тогда средняя скоростьт поступления пакета в очередль равна $\frac{N}d=550$
пакетов в секунду.
...
#### Решение
1. Когда первый пакет окажется в буфере машрутизатора, второй начнет приниматься
по первой линии, а первый -- отправляться по второй линии. Второй окажется на
маршрутизаторе через $\frac{L}{R_S}$, а первый -- у клиента -- через $d + \frac{L}{R_C}$.
После этого второй пакет еще $d + \frac{L}{R_C}$ будет идти до клиента, то есть
разница составит $\frac{L}{R_S}$. Второй пакет не будет ждать, так как певрый пакет
уйдет раньше ($\frac{L}{R_C}$), чем прийдет второй ($\frac{L}{R_S}$).
2. Теперь, второй пакет окажется на маршрутизаторе через $\frac{L}{R_S}$,
после приемки первого пакета, то есть раньше, чем первый уйдет с маршрутизатора
(через $\frac{L}{R_C}$). Тогда второй пакет будет ждать
в  очереди $\frac{L}{R_C}-\frac{L}{R_S}$ -- и именно столько серверу нужно
подождать, чтобы очереди не было.
...
#### Решение
1. Отправка объекта в $850000bit$ по каналу доступа в $15Mbit/s$ занимает
$\approx 57ms$.
2. Средняя задержка доступа составляет
$\frac{\Delta}{1-\Delta\cdot B} \approx 648ms$. В интернете запрос проводит еще $3000ms$,
то есть средее время ответа равно $3648ms$.
3. Запросы, попавшие в кеш, должны дойти до маршрутизатора $648ms$, а не попавшие
 -- еще и в интернет $3000ms$. Тогда среднее время ответа станет $648 + 0.4 * 3000 = 1848ms$.
