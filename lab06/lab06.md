...
#### Вопросы
![]({1DB2A30B-CE1D-4E81-AE19-3D2B25533D9C}.png)

1. Выберите один UDP-пакет. По этому пакету определите, сколько полей содержит UDP-заголовок.
   - 4
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   информации о содержимом полей в данном пакете.
   - по 2 байта (в правом окне с сырыми байтами сообщения)
3. Значение в поле Length (Длина) – это длина чего?
   - Длина всего udp пакета, с заголовками
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - В поле длины записано число не более $2^16-1=65535$, из этого 8 байт отведено под
   заголовок, итого $65527$ байт полезной нагрузки
...
![]({C34F0115-EE25-4906-9703-619B1DC0B23D}.png)

   - Сообщения отпралены с порта 49549 на порт 53 и назад
...
#### Скрины
На скриншоте из клиента виден файл foo.txt, полученный через фтп сервер. В логах
сервера видна отправка файла
