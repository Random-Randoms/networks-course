...
#### Вопросы
![alt text]({1DB2A30B-CE1D-4E81-AE19-3D2B25533D9C}.png)

1. Выберите один UDP-пакет. По этому пакету определите, сколько полей содержит UDP-заголовок.
   - 4: два порта, длина и контрольная сумма
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   информации о содержимом полей в данном пакете.
   - Справа, в структуре пакета, каждое поле занимает по два байта
3. Значение в поле Length (Длина) – это длина чего?
   - Длина всего пакета, включая хедеры
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - Максимальное значение длины (2 байта -- 65535 байт) минус размер заголовка (8 байт),
   то есть 65527 байт
5. Чему равно максимально возможное значение номера порта отправителя?
   - 65535 -- записывается в два байта
...
![alt text]({C34F0115-EE25-4906-9703-619B1DC0B23D}.png)
   - прямой пакет посылался с порта 49549 на порт 53, а обрбатный -- с порта 53 на
   порт 49549
...
#### Скрины
![]({267C0E3B-9473-4328-841D-B87FDA0E4688}.png)
На скрине с клиента виден файл `foo.txt` на ftp-сервере. В логах сервера видна
отгрузка этого файла клиенту
...
#### Демонстрация работы
Реализован клиент на сокетах.
В приложенном скрине происходит подключение к фтп-серверу на моем телефоне, выгрузка оттуда
изображения (приложено), и загрузка туда другого изображения
![alt text]({0EEFAC87-5AB7-44BA-B9E0-82ADC02AE6DE}.png)
