...
#### Демонстрация работы
Для спецификации используется формат `.toml`:
```rip-sim/sample-2.toml
# square 1-2-3-4 with additional host 5 connected with 1
hosts = [
    "192.168.0.1",
    "192.168.0.2",
    "192.168.0.3",
...
channels = [
    ...
    { from = "192.168.0.2", to = "192.168.0.3"},
    { from = "192.168.0.3", to = "192.168.0.4"},
    { from = "192.168.0.4", to = "192.168.0.1"},
    { from = "192.168.0.1", to = "192.168.0.5"},
]

```
Запуск без флага `--show-intermediate`, выводится конечное состояние:
![alt text]({8251C680-F07C-472A-B518-14958CFA7458}.png)
...
#### Демонстрация работы
Запуск с флагом `--show-intermediate`, выводятся промежуточные состояния (но на скрин не влезают).
![alt text]({FE997005-E53B-46B7-A688-A7E9A22B764D}.png)
